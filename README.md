...
| [`wachi-producto`](skills/wachi-producto) | El orquestador del proceso de PRODUCTO (gemelo pre-dev de wachi-fabrica): entiende qué trae el usuario, diseña la ruta mínima por ideación→definición→validaciones y aterriza todo en RumIAndo hasta la ficha READY_TO_BUILD | Perennia-Regeneracion · MIT |
| [`wachi-ideacion`](skills/wachi-ideacion) | Ideación anti-slop: grounding + generar-muchas/criticar-todas con verificador de contexto fresco, base citada obligatoria por idea, y las 6 preguntas forzadoras de demanda real | Propia; patrones de EveryInc CE (`ce-ideate`) y gstack (`office-hours`) · MIT |
| [`wachi-definicion`](skills/wachi-definicion) | Contrato de producto como UN artefacto que madura (requisitos R-IDs, actores, flujos, aceptación), right-sizing y confirmación de alcance antes de escribir | Propia; patrones de EveryInc CE (`ce-brainstorm`/`ce-plan`) · MIT |
| [`wachi-validacion-usuario`](skills/wachi-validacion-usuario) | Sesiones de validación con usuarios reales sobre RumIAndo: preparar desde el contrato, mirar-sin-demostrar, hallazgos tipados + decisiones, y cierre del loop a la state machine | Perennia-Regeneracion · MIT |
| [`wachi-validacion-interna`](skills/wachi-validacion-interna) | Panel interno pre-build multi-lente (negocio/diseño/ingeniería) con modo de alcance explícito, gates ADR+DoR, y veredictos de adopción de dos pisos (Adoptar/Probar/Esperar/Rechazar) | Propia; patrones de gstack (`plan-*-review`) y EveryInc CE (`ce-pov`) · MIT |
| [`wachi-compound`](skills/wachi-compound) | Captura de aprendizaje con criterio de aplicación explícito (cuándo sí / cuándo NO), enrutado por la frontera de datos, y poda del conocimiento viejo (mantener/actualizar/consolidar/reemplazar/borrar) | Propia; patrones de EveryInc CE (`ce-compound`) · MIT |
